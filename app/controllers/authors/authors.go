...
func Create(c *fiber.Ctx) error {
	item := entities.Author{}
	if err := c.BodyParser(&item); err != nil {
		return err
	}

	err := database.Connection().Model(&entities.Author{}).Create(&item).Error
	if err != nil {
		return err
	}
...
func Index(c *fiber.Ctx) error {
	items := []entities.Author{}
	err := database.Connection().Model(&entities.Author{}).Preload(clause.Associations).Find(&items).Error
	if err != nil {
		return err
	}
...
func Delete(c *fiber.Ctx) error {
	...
	err = database.Connection().Model(&entities.Author{}).Delete(&entities.Author{
		ID: int_value,
	}).Error
	if err != nil {
		return err
